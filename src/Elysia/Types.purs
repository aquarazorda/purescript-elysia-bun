...
-- | Set type for Elysia.
type Set
  = { status :: Int -> Effect Unit
    , setHeader :: String -> String -> Effect Unit
    }

-- | Context record representing request, response, and helper methods.
type Context t
  = { request :: Request
    , body :: String
    , query :: { | t }
...
defaultCookieOptions = CookieOptions { maxAge: oneYear, signed: false, path: "/" }
  where
